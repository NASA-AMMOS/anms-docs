...
Because the ANMS is deployed as a Podman Compose configuration, the only primary files present on the host are to configure compose, its use as a system service, and the system firewall.
...
`.env`:::
Environment configuration for the ANMS containers, including host port mapping options (see <<sec-deploy-containers>>).
`docker-compose.yml`:::
The actual Podman Compose configuration for the ANMS, which is used by the systemd service `podman-compose@anms`.
...
These use the local container network to communicate with the ANMS, while real remote agents will require the external network configuration to include port forwarding and host name resolution for the ANMS BP Agent.
...
These procedures assume that the user has a working copy of the ANMS source tree an intends to make some changes to the source or the configuration outside of a normal production deployment procedure (where build happens on a separate host than ultimate installation).
