...
UI Terminology::
Link:::
A browseable item within a web page which accesses a specific item on the same page or a different page.
Redirect:::
...
The following section provides an overview of the ANMS User Interface, organized by the capabilities provided by each of the tabs - Monitor, Agents, Build, Status, ADMs - shown at left side pop-out menu of the ANMS display.
...
The rightmost side contains the profile info link, along with a "Logout" link.
...
1. Send a pre-built string ARI to the agent(s)
...
[#fig-agent-manage-waiting]
.The Agent Management Waiting Indicator
image::images/agent-manage-waiting.png[]
...
[#fig-string-input]
.ARI Text Input Toggle
image::images/string-input.png[]

In the Text Input mode, the ANMS transcodes string ARIs to CBOR, which can then be sent to Agents. To perform this translation, toggle the switch at the top of the screen to select the ARI *Text Input* option.
...
.ARI Parameter Added to an AC concatenated in ARI text
...
It is used primarily for debugging purposes to correlate with backend log entries.
...
Clicking an ADM's download button will download the associated yang file.
...
For this sample workflow, the `num-exec-succeeded` EDD is selected with the `Inspect` control.
